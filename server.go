...
func runServer(serverParam ethrServerParam) {
	...
	if serverParam.protocol == KCP {
		...
		srvrRunKCPServer()
	} else if serverParam.protocol == RUDP {
		ui.printMsg("Listening on port %d for TCP & RUDP", gEthrPort)
		srvrRunRUDPServer()
...
func srvrRunRUDPServer() error {
	config := RUDPConfig{}
	err := parseRUDPJSONConfig(&config, "rudp.json")
	if err != nil {
		ui.printErr("parse rudp config error:", err)
	}
	switch config.Mode {
	case "normal":
		config.NoDelay, config.Interval, config.Resend, config.NoCongestion = 0, 40, 2, 1
	...
	case "fr3":
		config.NoDelay, config.Interval, config.Resend, config.NoCongestion = 0, 40, 3, 1
	case "fast":
		config.NoDelay, config.Interval, config.Resend, config.NoCongestion = 0, 30, 2, 1
	case "fast2":
		config.NoDelay, config.Interval, config.Resend, config.NoCongestion = 1, 20, 2, 1
	...
	if config.TCP { // tcp dual stack
		...
		lis, err := rudp.ServeConn(nil, config.DataShard, config.ParityShard, conn)
		if err != nil {
			ui.printErr("rudp.ServeConn: %v", err)
		}
		go srvrRunRUDPPacketHandler(lis, config)
	}

	lis, err := rudp.ListenWithOptions(gLocalIP+":"+gEthrPortStr, nil, config.DataShard, config.ParityShard)
	if err != nil {
		ui.printErr("rudp.ListenWithOptions: %v", err)
	}
	go srvrRunRUDPPacketHandler(lis, config)
	return nil
}
...
func srvrRunRUDPPacketHandler(listener *rudp.Listener, config RUDPConfig) {
	if err := listener.SetDSCP(config.DSCP); err != nil {
		ui.printErr("rudp SetDSCP:%v", err)
	}
	if err := listener.SetReadBuffer(config.SockBuf); err != nil {
		ui.printErr("rudp SetReadBuffer:%v", err)
	}
	if err := listener.SetWriteBuffer(config.SockBuf); err != nil {
		ui.printErr("rudp SetWriteBuffer:%v", err)
	}

	for {
		s, err := listener.AcceptRUDP()
		if err != nil {
			ui.printDbg("AcceptRUDP error: %v", err)
		}
		s.SetStreamMode(config.StreamMode)
		s.SetWriteDelay(false)
		s.SetNoDelay(config.NoDelay, config.Interval, config.Resend, config.NoCongestion)
