...
func ethrDialRUDP(addr string) (*rudp.UDPSession, error) {
	config := RUDPConfig{}
	...
	var sess *rudp.UDPSession
	if config.TCP {
		conn, e := tcpraw.Dial("tcp", addr)
		if e != nil {
			return nil, errors.Wrap(err, "tcpraw.Dial()")
		}
		sess, err = rudp.NewConn(addr, nil, config.DataShard, config.ParityShard, conn)
	} else {
		ui.printDbg("rudp.DialWithOptions addr=%v config.DataShard=%v config.ParityShard=%v block=%v", addr, config.DataShard, config.ParityShard, nil)
		sess, err = rudp.DialWithOptions(addr, nil, config.DataShard, config.ParityShard)
	}

	if err != nil {
		return nil, errors.Wrap(err, "rudp.NewConn or rudp.DialWithOptions")
	}

	ui.printDbg("rudp session set config=%+v", config)
	sess.SetStreamMode(config.StreamMode)
	sess.SetWriteDelay(false)
	sess.SetNoDelay(config.NoDelay, config.Interval, config.Resend, config.NoCongestion)
	sess.SetWindowSize(config.SndWnd, config.RcvWnd)
...
func ethrDialEx(p EthrProtocol, dialAddr, localIP string, localPortNum uint16, ttl int, tos int) (conn net.Conn, err error) {
	...
	if p == RUDP {
		ui.printDbg("ethrDialRUDP %v", dialAddr)
		return ethrDialRUDP(dialAddr)
	}
