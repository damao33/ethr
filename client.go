...
func runTest(test *ethrTest) {
	...
	} else if test.testID.Protocol == RUDP {
		if test.testID.Type == Bandwidth {
			RunBandwidthTest(RUDP, test, toStop)
		} else if test.testID.Type == Latency {
			if test.clientParam.BufferSizeRecv > 1 || test.clientParam.BufferSizeSend > 1 {
				go runLatencyTestEx(RUDP, test, gap, toStop)
			} else {
				go runLatencyTest(RUDP, test, gap, toStop)
			}
		}
	...
	if p == KCP {
		...
		go func() {
			for {
				...
				conv := conn.(*rudp.UDPSession).GetConv()
				rto := conn.(*rudp.UDPSession).GetRTO()
				srtt := conn.(*rudp.UDPSession).GetSRTT()
				srttvar := conn.(*rudp.UDPSession).GetSRTTVar()
				//https://datatracker.ietf.org/doc/html/rfc6298
				//https://www.cnblogs.com/ys-ys/p/10265254.html
