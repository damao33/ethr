...
func printBwTestDivider(p EthrProtocol) {
	if p == TCP {
		ui.printMsg("- - - - - - - - - - - - - - - - - - - - - - -")
	} else if p == UDP || p == KCP || p == QUIC || p == RUDP {
		ui.printMsg("- - - - - - - - - - - - - - - - - - - - - - - - - - - -")
	}
}
...
func printTestResult(test *ethrTest, seconds uint64) {
	if test.testID.Type == Bandwidth &&
		(test.testID.Protocol == TCP || test.testID.Protocol == UDP || test.testID.Protocol == KCP || test.testID.Protocol == QUIC || test.testID.Protocol == RUDP) {
		if gInterval == 0 {
